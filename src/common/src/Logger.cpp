...
#include <iostream>
const char* logLevelString[] = {"Debug", "Info", "Warn", "Error"};
...
void FileLogger::openLogFile() {
  ...
  fd_ = ::open(logFileName_.c_str(), O_RDWR | O_CREAT | O_APPEND,
               S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH);
}

bool FileLogger::addLogHeader(char* buffer, int size, const char* level,
                              int* psize, const char* file, const char* func,
                              int line) {
  // clock time
  timeval tv;
  ...
  int written = -1;
  written = snprintf(
      buffer, size, "[%d-%02d-%02d %02d:%02d:%02d.%03d][%s][%d][%s][%s:%d] ",
      1900 + now.tm_year, 1 + now.tm_mon, now.tm_mday, now.tm_hour, now.tm_min,
      now.tm_sec, (int)(tv.tv_usec / 1000), level, getpid(), func, file, line);
...
void FileLogger::log(LogLevel level, const char* file, const char* func,
...
void FileLogger::log(LogLevel level, const char* file, const char* func,
  ...
  int size = 0;
  bool ok = addLogHeader(logData_, LOG_MAX_SIZE, logLevelString[level_], &size,
                         file, func, line);
  if (!ok) {
    return;
  }
  ...
  // Add errno and strerror() if error. Check size to avoid security issue
  // https://github.com/ossrs/srs/issues/1229
  if (level_ == LogLevelError && errno != 0 && size < LOG_MAX_SIZE) {
    r0 =
        snprintf(logData_ + size, LOG_MAX_SIZE - size, "(%s)", strerror(errno));
