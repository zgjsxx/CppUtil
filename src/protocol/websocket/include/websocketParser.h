...
typedef int (*websocket_data_cb)(websocket_parser *, const char *at,
                                 size_t length);
...
// store mask's offset
void websocket_parser_decode(char *dst, const char *src, size_t len,
                             websocket_parser *parser);

// Apply XOR mask (see https://tools.ietf.org/html/rfc6455#section-5.3) and
// return mask's offset
uint8_t websocket_decode(char *dst, const char *src, size_t len,
                         const char mask[4], uint8_t mask_offset);
#define websocket_encode(dst, src, len, mask, mask_offset) \
  websocket_decode(dst, src, len, mask, mask_offset)
...
size_t websocket_build_frame(char *frame, websocket_flags flags,
